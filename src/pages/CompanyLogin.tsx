...
const CompanyLogin = () => {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    toast.success(mode === "trial" ? "Free trial started — 14 days, no card needed" : "Welcome back");
    nav("/company");
  };
          ...
          <form onSubmit={submit} className="space-y-4">
            {mode === "trial" && (
              <div>
                ...
                <Input value={form.company} onChange={(e) => setForm({ ...form, company: e.target.value })} placeholder="BlueCart Logistics" className="mt-1.5" />
              </div>
            )}
            <div>
              <Label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Work email</Label>
              <Input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} placeholder="ops@yourcompany.com" className="mt-1.5" />
            </div>
            <div>
              <Label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Password</Label>
              <Input type="password" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} placeholder="••••••••" className="mt-1.5" />
            </div>
